...
import { Calendar, Users, MessageCircle, Clock, CheckCircle, XCircle } from 'lucide-react';
import type { LucideIcon } from 'lucide-react';
import { getAppointmentStats, getAllCounselorAppointments, type Appointment } from '@/apis/counselor';
      ...
      try {
        ...
        if (statsResponse.success && statsResponse.data?.stats) {
          ...
          setStats({
            ...
            completed: statsData.breakdown.find((item: any) => item._id === 'confirmed')?.count || 0,
            pending: statsData.pending,
            cancelled: statsData.breakdown.find((item: any) => item._id === 'cancelled')?.count || 0
          });
        }

        // Process appointments (limit to 5 most recent)
        if (appointmentsResponse.success && appointmentsResponse.data?.appointments) {
          const appointments = appointmentsResponse.data.appointments.slice(0, 5);
      ...
      <div className="space-y-6 min-h-screen">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <a href="/counselor/session" className="text-purple-600 hover:text-purple-700 font-medium text-sm">
              View All
            </a>
          </div>
              ...
              {appointments.map((apt) => (
                <div key={apt._id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-semibold text-gray-800">{apt.user.username}</p>
                      <p className="text-sm text-gray-500">
                        {(() => {
                          // Handle both possible field name formats
                          const dateValue = (apt as any).date || (apt as any).appointmentDate;
                          const timeValue = (apt as any).time || (apt as any).appointmentTime;
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-medium capitalize ${getStatusStyles(apt.status)}`}>
                    {apt.status}
                  </span>
                </div>
              ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <a href="/counselor/schedule" className="bg-white rounded-xl p-6 shadow-sm hover:shadow-md transition-all text-left group">
            ...
            <h3 className="font-semibold text-gray-800 mb-2">Update Availability</h3>
            <p className="text-sm text-gray-600">Manage your schedule</p>
          </a>

          <a href="/counselor/session" className="bg-white rounded-xl p-6 shadow-sm hover:shadow-md transition-all text-left group">
          ...
          <a href="/counselor/profile" className="bg-white rounded-xl p-6 shadow-sm hover:shadow-md transition-all text-left group">
